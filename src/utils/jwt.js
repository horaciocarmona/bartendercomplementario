...
export const generateToken=(user)=>{ //seria el register
    const token=jwt.sign({user},process.env.PRIVATE_KEY_JWT,{expiresIn:'12h'})
    return token
}

export const authToken=(req,res,next)=>{ //seria el login
    //consulto el heder
    const authHeader=req.headears.authorization

    if (!authHeader) { //entra si no inicio sesion o vencio el token
